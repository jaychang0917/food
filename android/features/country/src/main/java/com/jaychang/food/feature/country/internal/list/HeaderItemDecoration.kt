...
internal class HeaderItemDecoration(
    ...
    private fun toItem(parent: RecyclerView, view: View): Item? {
        val items = (parent.adapter as ListAdapter<CountryItemViewModel, *>).currentList
        ...
        val hasSameInitial = preItem.name.value!![0].equals(item.name.value!![0], ignoreCase = true)
        val shouldDrawHeader = shouldShowHeader && (!hasSameInitial || itemPos == 0)
        return Item(shouldDrawHeader, item.name.value!![0], itemPos)
    }
